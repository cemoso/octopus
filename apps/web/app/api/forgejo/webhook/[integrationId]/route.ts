...
export async function POST(request: Request, { params }: { params: Promise<{ integrationId: string }> }) {
  ...
  const automatic = event === "pull_request" && ["opened", "reopened", "synchronized"].includes(payload.action ?? "");
  ...
  const mention = event === "issue_comment" && payload.action === "created"
    ...
    && /(?:@octopus(?:review|-review)?\b|(?:^|\s)\/octopus\b)/i.test(payload.comment.body);
  ...
  await prisma.webhookDelivery.upsert({ where, update: {}, create: {
