...
Forgejo setup: connect an HTTPS instance with a personal access token (read:user, write:repository, write:issue), then configure signed repository pull-request webhooks using the URL and secret from Settings > Integrations. A dedicated bot account is recommended. Octopus syncs repositories and posts review comments and commit statuses. Octopus Cloud reaches public HTTPS instances. Private LAN/VPN instances require self-hosted Octopus: the official self-host image has NEXT_PUBLIC_OCTOPUS_SELF_HOSTED=true baked in at build time; custom/prebuilt images can instead set OCTOPUS_SELF_HOSTED=true at runtime. Either is sufficient. Set FORGEJO_ALLOWED_PRIVATE_ORIGINS to comma-separated exact HTTPS origins. Web and review workers both need matching allowlists, DNS and network access. Internal CAs use NODE_EXTRA_CA_CERTS with a trusted PEM file; never disable TLS verification. Loopback, link-local and metadata endpoints remain denied. See https://octopus-review.ai/docs/self-hosting#forgejo. Native CLI agent setup remains GitHub-only. Enable issue comment webhook events for /octopus or @octopus PR comment requests. Self-hosting Forgejo does not keep review processing on that server: Octopus and configured AI services process repository content. See https://octopus-review.ai/docs/integrations#forgejo.
