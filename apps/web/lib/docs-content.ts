...
6. Open /repositories in Octopus and select the repository. Confirm Auto Review is enabled, turning it on if needed; this setting remains editable during preparation. Open a non-draft test pull request in Forgejo. Indexing and analysis run automatically when the review starts; manual preparation is optional. Follow the first-review guide at /dashboard and check /review-logs until a review completes, then read its comments and commit status in Forgejo. Keep the connector running for indexing and reviews.
