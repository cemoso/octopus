...
Octopus manages the webhook lifecycle — no manual setup needed.`,
      ...
      {
        heading: "Forgejo",
        text: `Connect an HTTPS Forgejo instance in Settings > Integrations using its URL and a personal access token. A dedicated bot account is recommended. Required token scopes: read:user, write:repository, write:issue. Repository admin access is needed to configure webhooks.
Octopus encrypts the token and syncs repositories where the account has admin access. Add a Forgejo webhook to each repository using the URL and secret shown in Octopus; enable pull request events. Signed events trigger reviews. Enable issue comment events too to request reviews with /octopus or @octopus comments on pull requests. Reviews post comments and commit statuses; manual re-review is available in Octopus.
Octopus Cloud connects to public HTTPS instances. For private LAN/VPN instances, run self-hosted Octopus using the official self-host image (NEXT_PUBLIC_OCTOPUS_SELF_HOSTED=true baked in at build time) or OCTOPUS_SELF_HOSTED=true at runtime for a custom/prebuilt image. Either self-host mode is sufficient. Set FORGEJO_ALLOWED_PRIVATE_ORIGINS to comma-separated exact HTTPS origins without paths or wildcards. Configure the same allowlist, DNS and network access on web and review workers. Forgejo must reach the Octopus webhook endpoint. For an internal CA, mount its trusted PEM file and set NODE_EXTRA_CA_CERTS in each process; do not disable TLS verification. HTTP, redirects, loopback, link-local and metadata endpoints remain blocked. See /docs/self-hosting#forgejo. Forgejo hosts the repositories, but Octopus and the configured AI services process code for indexing and reviews. Self-hosting Forgejo alone does not keep processing on your network. Native CLI agent setup remains GitHub-only. Setup: /docs/integrations#forgejo.`,
      },
      {
        heading: "Jira",
