...
mock.module("@octopus/db", () => ({ prisma: {
  ...
  pullRequest: { updateMany: ({ data, where }: { data: Record<string, unknown>; where: { headSha?: string } }): Action => ({ kind: "update", data, headSha: where.headSha }) },
...
const coverage = unknownReviewCoverage("github", "fixture");
