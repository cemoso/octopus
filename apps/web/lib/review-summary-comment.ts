...
import { createPullRequestComment, updatePullRequestComment, getInstallationToken } from "@/lib/github";
...
export async function publishReviewSummary(target: SummaryTarget): Promise<number | null> {
  ...
  return prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    if (id !== null) {
      try {
        await updatePullRequestComment(target.installationId, target.owner, target.repo, id, body, token, signal);
      } catch (error) {
        // A missing comment can be recreated. Authentication, rate-limit and
        // ambiguous network failures must not create a second summary.
        if (!(error instanceof Error) || error.message !== "Failed to update PR comment: 404") throw error;
        id = null;
      }
    }
    if (id === null) {
      id = await createPullRequestComment(target.installationId, target.owner, target.repo, target.prNumber, body, token, signal);
