...
Self-hosted onboarding now enters the HTTPS origin and token directly, then uses **Connect Forgejo**; the integrations guide and assistant content match that flow. Consumed connector payloads are scrubbed immediately. Recovery metadata uses renewable 60-second leases, capped at two hours, until the same review head and request version has a durable terminal outcome. Expired or failed metadata is eligible for cleanup after five minutes; the reconciliation hold survives metadata cleanup.
